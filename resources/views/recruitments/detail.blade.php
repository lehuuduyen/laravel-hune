                            ...
                            <div class="wrapper row" style="margin-top: 40px;">
                                    ...
                                    @else
                                        ...
                                        <div class="preview-pic tab-content">
                                            @for ($i = 0; $i < count($array_photo); $i++)
                                                @if ($i == 0)
                                                    <div class="tab-pane active" id="pic-{{ $i }}"><img style="height: 100%; width: 100%; object-fit: contain;" src="https://prdapp.hunegroup.com/fileupload/Recruitment/{{ $array_photo[$i] }}" /></div>
                                                @else
                                                    <div class="tab-pane" id="pic-{{ $i }}"><img style="height: 100%; width: 100%; object-fit: contain;" src="https://prdapp.hunegroup.com/fileupload/Recruitment/{{ $array_photo[$i] }}" /></div>
                                                @endif
                                            @endfor
                                        </div>
                                        <ul class="preview-thumbnail nav nav-tabs">
                                            @for ($i = 0; $i < count($array_photo); $i++)
                                                @if ($i == 0)
                                                    <li class="active"><a data-target="#pic-{{ $i }}" data-toggle="tab"><img style="height: 100%; width: 100%; object-fit: contain;" src="https://prdapp.hunegroup.com/fileupload/Recruitment/{{ $array_photo[$i] }}" /></a></li>
                                                @else
                                                    <li><a data-target="#pic-{{ $i }}" data-toggle="tab"><img style="height: 100%; width: 100%; object-fit: contain;" src="https://prdapp.hunegroup.com/fileupload/Recruitment/{{ $array_photo[$i] }}" /></a></li>
                                                @endif
                                            @endfor
                                        </ul>
                                    @endif
                                ...
                                <div class="details col-md-6">
                                    ...
                                    <h5 class="sizes">User_ID:
                                        <a href="{{ url('show-list-post?id='.$post->user_id.'&service=7') }}">
                                    ...
                                    @if (isset($count_rp) && count($count_rp) > 0)
                                        ...
                                        <p id="status-{{ $post->id }}">Set On/Off: 
                                            ...
                                            {{-- <a button href="{{ url('/recruitments/status?id='.$post->id.'&status='.$post->status) }}">Set On/Off Bài đăng</a> --}}
                                    @endif
